...
public:
  ...
  CTransform(const Vec2 &p, const Vec2 &s, const double a)
  ...
  void update() {
    ...
    for (auto &entity : m_entities) {
      if (entity->cTransform && entity->cShape) {
        entity->cShape->circle.setPosition(entity->cTransform->pos.x,
...
class Game {
    ...
    for (auto &e : m_entities.getEntities("enemy")) {
      ...
      if (e->cTransform) {
        ...
        if (e->cTransform->pos.x - radius < 0 ||
            e->cTransform->pos.x + radius > m_window.getSize().x) {
          e->cTransform->speed.x *= -1;
        }
        if (e->cTransform->pos.y - radius < 0 ||
            e->cTransform->pos.y + radius > m_window.getSize().y) {
  ...
  void spawnPlayer() {
    ...
    // TODO: Finish adding properties to player with correct values from config
    // file
    ...
    entity->cTransform =
        std::make_shared<CTransform>(Vec2(mx, my), Vec2(2.0f, 2.0f), 0);
  ...
  void spawnEnemy() {
    ...
    sf::Color fill = sf::Color(rand() % 255, rand() % 255, rand() % 255);

    sf::Color outline = sf::Color(fill.r / 2, fill.g / 2, fill.b / 2);

    entity->cTransform = std::make_shared<CTransform>(
        Vec2(rand() % (m_window.getSize().x - 2 * rad) + rad,
             rand() % (m_window.getSize().y - 2 * rad) + rad),
        Vec2(rand() % 5 - 2.5, rand() % 5 - 2.5), rand() % 360);

    entity->cShape =
        std::make_shared<CShape>(radius, courners, fill, outline, 4.0f);
  ...
  void spawnSmallEnemies(std::shared_ptr<Entity> entity) {
    ...
    for (int i = 0; i < points; i++) {
      ...
      enemy->cTransform = std::make_shared<CTransform>(entity->cTransform->pos,
                                                       speed, rand() % 360);

      enemy->cShape = std::make_shared<CShape>(
          16.0f, points, entity->cShape->circle.getFillColor(),
          entity->cShape->circle.getOutlineColor(), 4.0f);
  ...
  void spawnBullet(std::shared_ptr<Entity> entity, const Vec2 &mousePos) {
    ...
    bullet->cShape = std::make_shared<CShape>(
        10.0f, 100, sf::Color(255, 255, 255), sf::Color(255, 255, 255), 4.0f);
  ...
  void spawnSpecialWeapon(std::shared_ptr<Entity> entity,
    ...
    bullet->cTransform =
        std::make_shared<CTransform>(mousePos, Vec2(0, 0), 0.0f);

    bullet->cShape = std::make_shared<CShape>(
        30.0f, 30, sf::Color(255, 255, 255), sf::Color(255, 255, 255), 4.0f);

    bullet->cCollision = std::make_shared<CCollision>(30.0f);

    bullet->cLifespan = std::make_shared<CLifespan>(30);
  }

public:
  Game(const std::string &config) { init(config); }
  void run() {
    while (m_running) {
